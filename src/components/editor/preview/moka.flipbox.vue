<template>
    <div 
        ...
        :key="doc.id"
        :class="'overflow-hidden flex flex-no-wrap '+ classe(doc.css)" :style="doc.style + ' ' +  background(doc)" :ref="doc.id" @click="next(index)" @mouseover="index=1" @mouseleave="index=0">
            ...
            <template v-for="(block,i) in doc.blocks">
                 ...
                 <moka-element
                  ...
                  <moka-preview-single-container 
                    ...
                    :class="'absolute top-0 left-0 right-0 bottom-0 ' + getIndex(i)"
...
import MokaPreviewSingleContainer from '@/components/editor/preview/moka.preview.single.container'
    ...
    methods:{
        ...
        next(index){
            if ( !this.$attrs.master ){
                ...
                typeof index != 'undefined'  ? index ? this.index = 0 : this.index = 1: null
                this.index === 0 ? 
                    backCard = 1 : 
                        backCard = 0
