<template>
        <ul :class="'tree'" >
            ...
            <li v-if="expand" :class="'list-none pl-' + (parseInt($attrs.level)+1)" :key="block.id" v-for="(block,b) in doc.blocks">
                <moka-tree-elements :expand="expand" :level="$attrs.level+1" v-if="block.hasOwnProperty('blocks')" :doc="block"/>
                <div class="border flex items-center mb-1 py-1 hover:bg-gray-200" v-if="!block.hasOwnProperty('blocks')" @click="setCurrent(block)">
