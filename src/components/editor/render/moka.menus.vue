                            ...
                            <div v-if="currentFocus===i" class="flex flex-col">
                                ...
                                <div class="flex flex-row items-center">
                                    <select class="" v-model="items[i].link" @focus="currentFocus=i">
                                        <option v-for="(opt,o) in articles" :value="'/' + opt.slug">{{ opt.title }}</option>
                                    </select>  
                                    <i class="material-icons" @click="$apollo.queries.articles.refetch()">refresh</i>
                                </div>
                            </div>
...
export default {
    name: 'MokaMenuEditor',
    components: { draggable , MokaEditIcon },
    data:()=>({
        currentIndex: -1,
        currentFocus: -1,
        subIndex: -1,
