<template>
<div class="absolute top-0 bg-gray-700 text-white p-4 w-full z-50 cursor-pointer right-0">
        ...
        <div class="flex flex-row justify-start">
            <div>
                ...
                <div class="flex flex-row m-auto mb-2">
                    ...
                    <i class="material-icons border border-black text-xl font-bold h-6 w-6 mr-2 bg-transparent text-red-500" title="transparent" @click="$emit('color','','')">clear</i>
                    <div class="border border-black  w-4 h-4 mr-2 bg-white" title="white" @click="$emit('color','white','')"></div>
                    <div class="border border-black  w-4 h-4 mr-2 bg-black" title="black" @click="$emit('color','black','')"></div>
                </div>
                    ...
                    <div v-for="(colore,c) in colors">
                        <div :key="'color_' + c" class="flex flex-row mb-2">
                            <div 
                                ...
                                v-for="n in 9" 
                                :title="colore + '-' +  n*100"
                                :class="'bg-' + colore + '-' + n*100 + ' border border-black  w-4 h-4 mr-2'" @click="$emit('color',colore,n*100)">
                            </div>
                        </div>            
                    </div>
                </div>
