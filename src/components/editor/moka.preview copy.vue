...
<div :class="doc.css + ' w-screen'" :style="stile(doc) + ' ' + background(doc)" id="content">
    ...
    <div v-if="$attrs.category!='element'" v-for="(block) in doc.blocks" :class="responsive(block.css.css) + ' ' + block.css.container" :style="stile(block) + background(block)" :id="block.anchor" :key="block.id" :ref="block.id">
        <!-- 2nd level - BLOCKS LOOP --->
        
        <div container v-if="block && !block.hasOwnProperty('slider')" v-for="(row) in block.blocks" :class="'flex flex-col relative ' + responsive(row.css)" :style="stile(row) + background(row)" :id="row.anchor" :key="row.id" :ref="row.id">
            ...
            <div elements v-for="(element) in row.blocks" v-if="element && row && !row.hasOwnProperty('slider')" :key="element.id" :ref="element.id" :class="$layer(element)" class="z-2">
                <moka-element

                    :element="element"
                    v-if="element && element.type!='grid' && element.type != 'flex' && !row.hasOwnProperty('slider')" 
                    :coords="[]"
                    :loop="$attrs.loop"
                    :develop="false"/>
                ...
                <div flex v-else :class="responsive(element.css.css) + ' text-base ' + element.css.container" :style="stile(element) + ' ' + background(element)">
                    ...
                    <div v-for="(subrow) in element.blocks" :class="responsive(subrow.css)" :style="stile(subrow) + background(subrow)" :key="subrow.id" v-if="!element.loop && element.type!='flex'" :ref="subrow.id">
                        ...
                        <div v-if="subelement" v-for="(subelement) in subrow.blocks" :key="subelement.id">
                            <moka-element 
    ...
    <div class="flex flex-row z-top p-1 fixed right-0 top-0">
        <i v-if="$attrs.develop" class="material-icons nuxpresso-icon-circle mr-2" title="Save" @click="print">save</i>
        <i class="material-icons nuxpresso-icon-circle" title="Close" @click="$emit('close')">close</i>
    </div>
</div>
</template>
