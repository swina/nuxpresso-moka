...
<div class="w-full h-auto p-10 top-0 left-0 relative">
    ...
    <div :class="doc.css + ' p-1 border-2 border-transparent '" :style="stile(doc,true) + ' ' + background(doc)" id="content">
        ...
        <draggable v-model="doc.blocks">
            <div block v-model="block.blocks" v-for="(block,b) in doc.blocks" :class="'relative ' + responsive(block.css.css) + ' ' + block.css.container" :style="stile(block) + background(block)" :key="block.id" :ref="block.id">
                    ...
                    <draggable container v-model="block.blocks" container v-for="(row,r) in block.blocks" :class="'relative z-3 flex flex-col h-full ' + responsive(row.css)" :style="stile(row) + background(row)" :key="row.id" :ref="row.id">
                        ...
                        <draggable v-model="row.blocks">
                            <!--<div v-for="(element,e) in row.blocks" class="relative z-4" v-if="!row.hasOwnProperty('slider')" :key="element.id" >-->
                                ...
                                <div v-model="element.blocks" :key="element.id" flexelement v-else  :class="'mt-4 relative text-white ' + responsive(element.css.css) + ' ' + element.css.container" :style="background(element)">
                                    ...
                                    <draggable v-if="subrow.type==='blocks'" v-model="element.blocks" subcontainer v-for="(subrow,s) in element.blocks" :class="'relative  ' + responsive(subrow.css)" :style="stile(subrow) + background(subrow)" :key="subrow.id">
                                    ...
                                    <div :class="'absolute transform scale-y-102 scale-x-102 border border-orange-600 w-full top-0 left-0 bottom-0 right-0 border-dashed z-4 ' + active(element.id)" style="min-height:1.4rem;" @click="setCurrent([b,r,e],element)">
                                        ...
                                        <i class="material-icons nuxpresso-icon-circle text-sm ">highlight_alt</i>
                                        </div>
                                    </div> 
                    ...
                    <!-- SLIDER BLOCK -->
                    <moka-slider v-if="block.hasOwnProperty('slider')" :develop="true" :embeded="true" :doc="block"/>   
                    ...
                    <div :class="'absolute transform scale-y-110 py-1 scale-x-107 border-2 border-blue-600  border-dashed z-2 top-0 left-0 bottom-0 right-0 ' + active(block.id)" @click="setCurrent([b],block)">
                        <i class="absolute top-0 right-0 transform scale-100 material-icons nuxpresso-icon-circle text-sm -mt-4">view_columns</i>
                    </div>  
            </div>  
        </draggable>
    </div>
    ...
    <div :class="'absolute border-2 border-green-600  border-dashed z-1 top-0 left-0 bottom-0 right-0 ' + activeDoc(doc.id)" @click="setCurrent([],doc)">
        <i class="absolute z-top top-0 right-0 transform scale-100 material-icons nuxpresso-icon-circle text-sm -mt-4">web</i>
    </div>
    --> 
